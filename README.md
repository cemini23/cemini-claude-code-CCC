...
Agent-harness meta-wiki in the Cemini federation alongside `osint-wiki`, `gambling-wiki`, `game-dev-wiki`, `cybersecurity-wiki`, `image-gen-wiki`, `seo-wiki`, and `3d-printing-wiki`.
...
**Eight** wikis + **Cemini Financial Suite** (private trading stack). Cross-links: `@<alias>/path/to/page.md` (`CLAUDE.md` → Related Wikis). Lint resolves paths on a maintainer machine with all repos checked out.
