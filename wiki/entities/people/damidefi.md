---
title: DamiDefi — Obsidian + Claude workflow author (X)
type: entity
tags: [people, x-twitter, obsidian, claude-code]
keywords: [damidefi, five-tool-stack, obsidian-vault, notebooklm]
related:
  - concepts/five-layer-operator-stack.md
  - concepts/claude-obsidian-notebooklm-bridge.md
  - sources/trading-posts-compilation-6-2026-05-29.md
  - sources/trading-posts-compilation-20-2026-05-27.md
  - sources/trading-posts-compilation-17-2026-06-03.md
  - entities/tools/kimi-agent-swarm.md
  - concepts/subagent-orchestration.md
maturity: draft
created: 2026-05-30
updated: 2026-06-03
cross-wiki-source: "@osint-wiki/entities/people/damidefi.md"
---

## Relations

- `@concepts/five-layer-operator-stack.md` — K78 Post 4 five-tool stack
- `@concepts/claude-obsidian-notebooklm-bridge.md` — K69 Posts 17–18 vault + NotebookLM
- `@sources/trading-posts-compilation-6-2026-05-29.md` — K78 provenance
- `@sources/trading-posts-compilation-20-2026-05-27.md` — K69 provenance

Cross-wiki: `@osint-wiki/entities/people/damidefi.md` (canon if present).

## Raw Concept

X author cited in Posts.docx batches for **Claude + Obsidian operator workflows** and the five-layer tool stack. CCC catalogs patterns only — not a tool vendor.

## Narrative

Referenced in K69 (vault thinking, NotebookLM CLI) and K78 (five-tool stack). K97 Post 16: **Opus + Kimi agent swarm** screening 50 YC startups — orchestration exemplar mapped to `@concepts/subagent-orchestration.md` and `@entities/tools/kimi-agent-swarm.md` (reference only). Workflow ideas are stolen; `@entities/tools/claude-obsidian.md` plugin remains **DO NOT ADOPT**.

## Snippets

> @DamiDefi — five-tool stack (Claude / Obsidian / Hermes / Kimi / Cursor).
> — [Source: @osint-wiki/sources/trading-posts-compilation-6-2026-05-29.md Post 4]
