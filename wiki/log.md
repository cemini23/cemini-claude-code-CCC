...
- **K278** InterSAGE (2608.13030) — trust-native IoA protocol: **Agent Identity Card** (4-D binding: developer × code package × operator × deployment context), **DID-bound capability VCs**, **monotonic capability attenuation** + two-tier access control, **kernel-mediated audit trails** (no ledger). **ADOPT** (Atto-priority); **no public SPDX clone** → policy only.
- **K279** MARC-v1 (2608.13476) — deterministic multi-agent clinical coordination. Steal **decomposer-from-NL**, **stage-wise failure attribution**, role-specialized pipeline. `Penn-RAIL/MARC-v1` **MIT** (~19MB) → **GO REFERENCE** `.local/adopts/MARC-v1`; clinical **wont_wire**.
- **K280** Vero (2608.13522) — first **repo-level joint impl+proof** verified synthesis bench (43 Lean 4 multi-module instances); **unsat/incorrectness audit** (eval must survive being told its spec/reference is wrong). **ADOPT**; `sunblaze-ucb/vero` **Apache-2.0** (~13MB) → **GO REFERENCE** `.local/adopts/vero`; runtime **wont_wire** (Lean bench).
- **K281** AutoDesign (2608.13560) — meta-harness optimizer from rollout feedback (PosterBench +7.45 over Claude Design; +12.4% avg harness transfer). Steal **with HITL / external eval contract** — pairs K162, no closed self-rewrite of pass criteria. `Yaxin9Luo/AutoDesign` LICENSE **MIT on disk** (gh API NOASSERTION; ~107MB) → **GO REFERENCE** `.local/adopts/AutoDesign`; poster/image-gen **wont_wire**.
- Phase-0 scripts `adopt_k277/278/279/280/281_phase0.sh` all green; policy wires extended `cemini-phase1-policy-wires.mdc` §K277–K281 + `ccc-k277-k281-phase1-wires.mdc`. **Dual-ID note:** Cybersec K277 = RSM (CCC K276); this batch's CCC K277 = Labels-Are-Not-Endpoints.
- Briefs: prod SCP'd (`2026-08-14_k277-k281-harness-wave-prod.md` + hands-on); routed **Atto (InterSAGE-led)**, Cybersec (treatment leakage + InterSAGE identity), Poker (light eval-integrity), GuruWatcher (identity/persona vs service callers).
- 5 PDFs archived to egress-fi; inbox empty. Local REFERENCE clones: MARC-v1, vero, AutoDesign (all gitignored `.local/adopts/`).
