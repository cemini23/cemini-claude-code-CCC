---
title: K124 — harness policy hands-on (brief)
type: source
tags: [source, brief, harness, policy, k124]
keywords: [k124, evidence-artifacts, rubric-compact]
related:
  - concepts/multi-agent-evidence-artifact-layers.md
  - sources/brief-k124-harness-policy-hands-on-2026-06-24.md
  - concepts/self-compacting-adaptive-context.md
  - concepts/procedural-memory-transfer-after.md
  - entities/patterns/cpr-context-compression.md
  - sources/brief-k122-k126-federation-cursor-harness-adoptions-2026-06-25.md
maturity: validated
read_status: deep-read
created: 2026-06-24
updated: 2026-06-29
---

## Relations

- @concepts/multi-agent-evidence-artifact-layers.md
- @entities/patterns/cpr-context-compression.md

## Raw Concept

| Field | Value |
|-------|-------|
| **Brief** | `briefs/2026-06-24_k124-harness-policy-hands-on.md` |
| **Adopted** | 2026-06-24 |

## Narrative

K124 pattern steals into rules, CLAUDE.md, skill-vetting, CPR, flaw template. No tool installs.

## Sources

- `briefs/2026-06-24_k124-harness-policy-hands-on.md`
