...
1. **Priority stack** — **#1 Atto** · **#2 GuruWatcher** · **#3 CeminiDFS** · **#4 CeminiParlays** · then wikis always-on.
2. **Poker arena retired** — do not score `poker-arena overlap`; do not route briefs to `devfun-poker-arena`.
3. **`ceminiparlays overlap`** — SGP / parlay EV, de-vig, copula, pick'em slip ranking. Path: `/Users/claudiobarone/Projects/CeminiParlays`.
4. CeminiDFS and CeminiParlays are peers after #1/#2: DFS projection/lineup/BBM → CeminiDFS; parlay/SGP/pick'em → CeminiParlays (do not merge stacks).
5. Gemini runtime paste updated: `prompts/deep-research-multi-wiki-eval-v10-gemini-runtime.md`.
